import Head from "next/head";
import Image from "next/image";
import { MdOutlineDoubleArrow } from "react-icons/md";

export default function Home() {
  return (
    <div>
      <Head>
        <title>Malis.</title>
        <meta name="description" content="Generated by create next app" />
        <link rel="icon" href="/favicon.ico" />
      </Head>
      <main>
        <section className="max-w-[1440px] mx-auto h-screen flex flex-col justify-evenly text-center">
          <h1 className="text-[40px] font-medium sm:text-[70px] md:text-[100px] lg:text-[140px]  ">
            Be Mini<span className="text-[#82AB2C]">malis</span>m.
          </h1>
          <div className="relative h-[300px] lg:h-[500px]">
            <Image
              src={"/landing-page-picutre-no-bg.webp"}
              alt="Minimalistic modern sofa"
              layout="fill"
              objectFit="cover"
              priority
            />
          </div>
          <button className="text-[18px] font-bold hover:text-gray-700 transition self-end flex items-center ">
            EXPLORE NOW <MdOutlineDoubleArrow />
          </button>
        </section>
      </main>
    </div>
  );
}
